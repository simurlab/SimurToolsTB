function plotSignalsWithColorBands(X, labelsTS, fs)

    [N, numSignals] = size(X);
    t = (0:N-1) / fs;

    uniqueClusters = unique(labelsTS);
    uniqueClusters(uniqueClusters < 0) = [];  % evitar -1 si hay DBSCAN
    K = numel(uniqueClusters);

    colors = lines(K);   % paleta

    % Preparar figura
    figure;
    signalNames = {'Ax','Ay','Az','Gx','Gy','Gz'};

    % Determinar límites de cada cluster
    ...
    % LEYENDA
    % ============
    % Crear handles invisibles para cada color
    hold on;
