...
for k = 1:numel(archivos)
    ...
    for p = 1:numel(sensores)
        ...
        % ------------------------------------------------------------------
        try
            [a_cal, g_cal] = carga_IMUstd(fullpath, sensor_id);
        catch
        ...
        if isempty(ic) || isempty(fc)
            warning('No se detectaron eventos en %s (%s). Se omite.', ...
                archivos(k).name, sensor_id);
            continue;
        end
        ...
        % ------------------------------------------------------------------
        resultados(end+1, :) = { ...
            archivos(k).name, sensor_id, numel(ic), ...
...
if isempty(resultados)
    disp('No se generaron resultados. Revisa la estructura de los archivos .mat.');
    return;
end
