% CARGA_INTENTO Funcion para cargar y orientar en formato ISB los datos 
% de un intento. Supone que hay un archivo con datos tipo BD-InnovaIM 
% accesible, dado por su ID.
% Utiliza la funcion calibra_anatomical. 
%
% Sintax: [a_cal, g_cal, quat_cal]=carga_intento(IDexp, IDsensor, Visualiza)
%
% Parametros de entrada:
%    IDexp              - Código de identificación de la prueba (letra+numero)
%    IDSensor           - Código que identifica el sensor () 
...
function [a_cal, g_cal, quat_cal, IMUmeta]=carga_intento(IDexp,IDsensor,Visualiza)
...
switch numArgs
    case 0
        prompt = "Introduzca el ID del experimento [a0101]: ";
        txt0 = input(prompt,"s");
        IDexp=txt0;
        if isempty(txt0)
            IDexp = 'a1';
        ...
        prompt = "Introduzca el Sensor de interés (1/) [1]: ";
...
IMU=eval(IDsensor);
IMUmeta=eval([IDsensor '_metadata']);

%Rcal=Rcalib(queSensor,:);
Rcal=IMUmeta.orientacion;
